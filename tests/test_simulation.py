...
def test_magnus_evolution_goes_fine_for_low_pulse_duration():
    spin_par = {'quantum number' : 1,
                'gamma/2pi' : 1.}
...
def test_j_coupling_refocusing_sequence():
    ...
    pulse_mode = pd.DataFrame([(25., 2., 0., np.pi/2, 0)], 
                              columns=['frequency', 'amplitude', 'phase', 'theta_p', 'phi_p'])
    ...
    assert np.all(np.isclose(initial_dm.full(), j_evolved_dm.full(), rtol=2, atol=5e-2))


def test_magnus_j_coupling_refocusing_sequence():
    spin_par1 = {'quantum number' : 1/2,
                 'gamma/2pi' : 1.}
    ...
    final_dm = evolve(spins, h_unperturbed, dm_3, magnus,
                      pulse_mode, pulse_time=0.1, \
                      picture='IP', order=2)
...
def test_FID_signal_decays_fast_for_small_relaxation_time():
    ...
    dm_evolved = evolve(spin, h_unperturbed, dm_0, mesolve,
                        mode, pulse_time=np.pi, \
                        picture='IP', \
                        n_points=10)
...
def test_magnus_FID_signal_decays_fast_for_small_relaxation_time():
    spin_par = {'quantum number' : 2,
                'gamma/2pi' : 1.}
    ...
    dm_evolved = evolve(spin, h_unperturbed, dm_0, magnus,
                        mode, pulse_time=np.pi, \
                        picture='IP', \
                        n_points=10, order=2)
...
def test_pure_zeeman_FID_is_periodic_for_long_relax_times():
    ...
    mode = pd.DataFrame([(nu, 1., 0., np.pi/2, 0)], 
                        columns=['frequency', 'amplitude', 'phase', 'theta_p', 'phi_p'])
...
def test_magnus_pure_zeeman_FID_is_periodic_for_long_relax_times():
    spin_par = {'quantum number' : 3/2,
                'gamma/2pi' : 1.}
...
def test_opposite_fourier_transform_when_FID_differ_of_pi():
    ...
    t, signal2 = FID_signal(spin, h_unperturbed, dm_evolved, acquisition_time=250, T2=100, phi=np.pi)
