...
class Operator:
    ...
    def cast_to_density_matrix(self):
        """
        Returns an object of the class DensityMatrix initialised with the matrix of the owner Operator object, if all the properties of a density matrix are satisfied.
    ...
    def free_evolution(self, stat_hamiltonian, time):
        """
        Tries to cast the operator into the type density_matrix (using method cast_to_density_matrix), and in case of success returns this object evolved through the time time under the effect of stat_hamiltonian, calling the method of DensityMatrix with the same name.
    ...
    def expectation_value(self, DensityMatrix):
        """
        Tries to cast the Operator into the type Observable (using method cast_to_observable), and in case of success returns its expectation value in the state represented by DensityMatrix, calling the method of Observable with the same name.
  
        See the description of Observable.expectation_value below for details.
        """
        ob = self.cast_to_observable()
        return ob.expectation_value(DensityMatrix)


# Objects of the class DensityMatrix are special Operator objects characterised by the following properties:
# i) Hermitianity;
# ii) Unit trace;
# iii) Positivity
...
The expectation value of an observable of a quantum system in a certain state is conventionally computed as the bra-operator-ket product of the observable's operator in between the state vector. Alternatively, one can exploit the density matrix representation of the state and find the expectation value as the trace of the product of the density matrix and the observable's operator.
    ...
    def expectation_value(self, DensityMatrix):
        """
        Returns the expectation value of the observable calculated in the state represented by DensityMatrix.
        
        Parameters
        ----------
        - DensityMatrix: DensityMatrix (or any Operator which can be cast to DensityMatrix)
                          State of the system.
